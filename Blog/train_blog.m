function [p,w] = train_blog(ext,trn,mu_w,lambda_p,lambda_w,N_iter)
...
p=rand(N,1);
w=ones(N,1);
%lambda_p=1e-6;lambda_w=1e-6;N_iter=500;mu_p=1;
for n=1:N_iter 
      ...
      %gradient p;
      grad_p=(first_term_p+second_term_p)/T_trn+1000*sign(p);%+2*mu_p*sum(repmat(p,1,T_trn)-trn_aplus',2);
